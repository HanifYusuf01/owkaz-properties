            ...
            {projects.map((project, idx) => (
              <div key={project.id} onClick={() => setSelectedProject(project)} className="bg-white border border-border rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-200 cursor-pointer">
                ...
                <div className="p-5">
                  ...
                  <div className="text-xs text-muted mb-4">📍 {project.location}</div>

                  {/* Stats */}
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    <div className="bg-cream rounded-lg px-2 py-2 text-center">
                      <div className="text-xs font-bold text-navy">{project.totalUnits}</div>
                      <div className="text-[10px] text-muted">Total Units</div>
                    </div>
                    <div className="bg-cream rounded-lg px-2 py-2 text-center">
                      <div className={`text-xs font-bold ${project.availableUnits === 0 ? 'text-muted' : 'text-teal'}`}>
                        {project.availableUnits}
                      </div>
                      <div className="text-[10px] text-muted">Available</div>
                    </div>
                    <div className="bg-cream rounded-lg px-2 py-2 text-center">
                      <div className="text-xs font-bold text-navy">{project.progress}%</div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <div className="text-xs text-muted">Price from</div>
                  ...
                  {project.features?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {project.features.slice(0, 3).map((f) => (
                        <span key={f} className="text-[10px] bg-surface border border-border text-muted px-2 py-0.5 rounded-full">{f}</span>
                      ))}
                      {project.features.length > 3 && (
                        <span className="text-[10px] bg-surface border border-border text-muted px-2 py-0.5 rounded-full">+{project.features.length - 3} more</span>
                      )}
                    </div>
                  )}
